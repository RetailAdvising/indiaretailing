                ...
                return (
                    ...
                    <div className={`overflow-hidden cursor-pointer flex ${isHome ? 'flex rounded-[10px] h-[160px]' : ' pb-[20px]'}  bg-white gap-[10px]   ${flex}`} onClick={() => router.push(`${isRoute ? isRoute + res.route : '/' + router.asPath.split('/')[1] + '/' + res.route}`)} key={index}>
                        ...
                        <div className={`flex flex-col ${isHome ? 'p-[10px] justify-between gap-[5px]' : 'leading-[2] px-[10px] min-h-[185px]'} `}>
                            <h4 className={`font-[700] ${isHome ? 'text-[14px] ' : 'text-[16px] py-[10px]'} line-clamp-2 text-[#39364F] !capitalize ${nunito.className}`}>{res.title} </h4>
                            <p className={`sub_title line-clamp-2`}>{res.description}</p>
                            {/* <p className={`sub_title pt-[5px]`}>{res.end}</p> */}
                            <div className={`flex gap-[10px] items-center ${isHome ? '' : 'pt-[10px]'} `}>
                                <p className={`flex gap-[5px] items-center `}><Image src="/calendar.svg" className={`object-contain`} objectFit='contain' height={15} width={20} alt={res.title} /> <span className={`light_text pt-[2px] ${nunito.className}`}>{res.start_date}</span></p> {res.locations && <span className='h-[18px] w-[2px] mx-[6px] bg-[#ddd]'></span>}
                                {res.locations && <p className={`flex gap-[5px] items-center`}><Image src="/location.svg" className={`object-contain`} objectFit='contain' height={10} width={20} alt={res.title} /> <span className={`light_text ${nunito.className}`}>{res.locations[0] && res.locations[0].event_location}</span></p>}
                            </div>
