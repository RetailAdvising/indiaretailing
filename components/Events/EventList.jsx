        ...
        <>
            ...
            {data && data.map((res, index) => {
                return (
                    ...
                    <div className={`overflow-hidden cursor-pointer flex ${isHome ? 'flex rounded-[5px] border h-[150px]' : ' pb-[20px]'} border_bottom bg-white gap-[10px]   ${flex}`} onClick={() => router.push(`${isRoute ? isRoute + res.route : '/'+router.asPath.split('/')[1]+'/'+res.route}`)} key={index}>
                        ...
                        <div className={`flex flex-col ${isHome ? 'p-[10px] justify-between gap-[3px]' : 'leading-[2] px-[10px] min-h-[185px]'} `}>
                            <h4 className={`font-semibold ${isHome ? 'text-[14px]' : 'text-[16px] py-[10px]'}  text-[#39364F] `}>{res.title} </h4>
                            <p className={`sub_title line-clamp-2`}>{res.description}</p>
                            ...
                            <div className={`flex gap-[5px] items-center ${isHome ? '' :'py-[10px]'} `}><p className={`primary_text font-semibold`}>Register Now</p>
                                <Image src="/arrowrightprimary.svg" className={`h-[17px] w-[17px] p-0.5`} height={14} width={14} alt={'res.title'} />
                            </div>
                        </div>
                    </div>
