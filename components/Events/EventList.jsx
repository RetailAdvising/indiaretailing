        ...
        <>
            ...
            {data && data.map((res, index) => {
                return (
                    ...
                    <div className={`overflow-hidden cursor-pointer flex ${isHome ? 'flex rounded-[5px] border h-[160px]' : ' pb-[20px]'} border_bottom bg-white gap-[10px]   ${flex}`} onClick={() => router.push(`${isRoute ? isRoute + res.route : '/'+router.asPath.split('/')[1]+'/'+res.route}`)} key={index}>
                        <div className='flex-[0_0_calc(40%_-_10px)]'>
                            <Image loading="lazy" blurDataURL={'/empty_state.svg'} placeholder='blur' style={{ background: imageBackground}} height={100} width={200} alt={res.title} src={!check ? check_Image(res.thumbnail_path) : res.image} className={`${height} ${width} ${isHome ? '' : 'rounded-[10px]'}`} />
                        </div>
                        <div className={`flex flex-col ${isHome ? 'p-[10px] justify-between gap-[5px]' : 'leading-[2] px-[10px] min-h-[185px]'} `}>
                            ...
                            <div className={`flex gap-[10px] items-center ${isHome ? '' : 'pt-[10px]'} `}>
                                <p className={`flex gap-[5px] items-center `}><Image src="/calendar.svg" className={`object-contain`} objectFit='contain' height={15} width={20} alt={res.title} /> <span className={`light_text pt-[2px]`}>{res.start_date}</span></p> {res.locations && <span className='h-[18px] w-[2px] mx-[6px] bg-[#ddd]'></span>}
                                {res.locations && <p className={`flex gap-[5px] items-center`}><Image src="/location.svg" className={`object-contain`} objectFit='contain' height={10} width={20} alt={res.title} /> <span className={`light_text `}>{res.locations[0] && res.locations[0].event_location}</span></p>}
                            </div>
