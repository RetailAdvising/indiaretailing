...
export default function EventCards({ data, flex,height,width,imgFlex,islanding,route }) {
    ...
    return (
        <>
            {data && data.map((res, index) => {
                ...
                return (
                    <div className={` flex flex-col rounded-[10px] h-[390px]  border cursor-pointer ${flex}`} onClick={() => islanding ? router.push(`${router.asPath}/${route}/${res.name}`) :  router.push(`${router.asPath}/${res.name}`)} key={index}>
                        <div className={``} >
                        ...
                        <div className={`flex flex-col p-[10px] h-full justify-between`}>
                            ...
                            <div className={`flex gap-[5px] items-center pt-[5px]`}>
                                <p className={`flex gap-[5px] border_right pr-[10px]`}><Image src={'/calendar.svg'} objectFit='contain' height={15} width={20} alt={res.title} /> <span className={`light_text`}>{res.start_date}</span></p>
                                <p className={`flex gap-[5px]`}><Image src={'/location.svg'} objectFit='contain' height={15} width={20} alt={res.title} /> 
