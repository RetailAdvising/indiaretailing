            ...
            {data.map((res, index) => {
                return (
                    <Link key={index} className={`${flex}`} href={isHome ? isHome + res.route : '/' + router.asPath.split('/')[1] + '/' + res.route}>
                        ...
                        <div className={` ${big ? 'relative' : ''} ${isHome ? 'lg:mb-[15px]' : isList ? 'h-full' : 'h-[235px]'} cursor-pointer relative`}>
                            ...
                            <Image src={'/irprime/youtube.svg'} className={`absolute ${big ? 'bottom-[70px] left-[10px]': isList ? 'bottom-[55px] left-[5px]' : 'bottom-[60px] left-[5px]'}  ${isList ? '' : 'md:bottom-[60px]'}  object-contain h-[20px] w-[30px]`} height={100} width={100} alt={res.title} />
