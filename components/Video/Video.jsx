                ...
                return (
                    <Link key={index} className={`${flex}`} href={isHome ? isHome + res.route : '/' + router.asPath.split('/')[1] + '/' + res.route}>
                        ...
                        <div className={` ${big ? 'relative' : ''} ${vh ? vh : ''} ${isHome ? 'lg:mb-[20px] ' : isList ? 'h-full' : 'h-[235px]'} cursor-pointer relative`}>
                            ...
                            <p className={`pt-[10px] text-[14px] md:text-[13px] ${big ? 'text-[17px] lg:absolute bottom-[15px] mx-[10px] leading-[22px] font-[500]' : ''} line-clamp-2 ${isBg ? 'text-white' : ''} !font-[700] ${nunito.className}`}>{res.title}</p>
                        </div>
                    </Link>
                )
            })}
