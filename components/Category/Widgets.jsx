            ...
            {data.title != 'Videos' && data.data.slice(0, 6).map((res, i) => {
              return (
                <div key={index} onClick={()=> checkRoute(res,data)} className='flex cursor-pointer h-[100px] gap-[10px] border rounded-[5px] p-[10px]'>
                  ...
                  <div>
                    {(res.primary_text && res.secondary_text) && <p className={`flex gap-2 !mb-[5px] items-center py-[5px] line-clamp-1`}><span className={`primary_text leading-normal tracking-wider !text-[10px] line-clamp-1`}>{res.primary_text}</span> <span className="h-[10px] w-[1px] bg-[#6f6f6f]"></span> <span className='secondary_text line-clamp-1'>{res.secondary_text}</span></p>}
                    ...
                    {data.title == 'Events' &&
                      <div className={`flex gap-[5px] items-center lg:pt-[10px]`}>
                        <p className={`flex !mb-0 items-center gap-[5px] md:flex-direction`}><Image src={'/calendar.svg'} className='md:hidden h-[15px] w-[13px] !p-0' objectFit='contain' height={25} width={20} alt={res.title} />  <span className={`light_text !text-[10px]`}>{res.start_date}</span></p>{res.locations && <span className='h-[18px] w-[2px] mx-[6px] bg-[#ddd]'></span>}
                        {res.locations && <>
                          <p className={`flex !mb-0 flex-wrap items-center gap-[5px]`}><Image src={'/location.svg'} className='md:hidden !p-0' height={15} width={20} alt={res.title} />
                            ...
                            {res.locations.slice(0, 1).map((item, index) => {
                              return (
                                <span key={index} className={`light_text !text-[10px]`}>{item.event_location}</span>
                              )
                            })}
                          </p>
                        </>}
