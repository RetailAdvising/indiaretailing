            ...
            {data && data.map((res, index) => {
                return (
                    ...
                    <div key={index} onClick={() => checkRoute(res)} className={`${flex} flex cursor-pointer gap-[15px] ${(index != data.length - 1 && !isMp) ? 'pb-[10px]' : (isMp && index != data.length - 1) ?
                        ...
                        {(res.primary_text && res.secondary_text && isTop) && <p className={`flex line-clamp-1  ${exclusives.title_top}  items-center absolute`}><span className={`primary_text ${nunito.className} pr-[8px] line-clamp-1`}>{res.primary_text}</span> {res.secondary_text && <span className='h-[10px] w-[1px]  bg-[#6f6f6f]'></span>} <span className={`pl-[8px] ${nunito.className} line-clamp-1 secondary_text`}>{res.secondary_text}</span></p>}
                        {/* ${check ? '' : 'basis-1/4'} */}
                        ...
                        {res.doc_type == 'Community Event' ?
                            <div className='flex flex-col'>
                                {res.title && <h6 className={`title  pt-[5px] ${titleClamp ? titleClamp : 'line-clamp-1'} ${nunito.className}`}>{res.title ? res.title : ''}</h6>}
                                <div className={`flex gap-[5px] items-center lg:pt-[10px]`}>
                                    <p className={`flex items-center gap-[5px] md:flex-direction`}><Image src={'/calendar.svg'} className='md:hidden h-[15px] w-[13px]' objectFit='contain' height={25} width={20} alt={res.title} />  <span className={`light_text ${nunito.className}`}>{res.start_date}</span></p>{res.locations && <span className='h-[18px] w-[2px] mx-[6px] bg-[#ddd]'></span>}
                                    {res.locations && <>
                                        <p className={`flex flex-wrap items-center gap-[5px]`}><Image src={'/location.svg'} className='md:hidden' height={15} width={20} alt={res.title} />
                                            ...
                                            {res.locations.slice(0, 1).map((item, index) => {
                                                return (
                                                    <span key={index} className={`light_text ${nunito.className}`}>{item.event_location}</span>
                                                )
                                            })}
                                        </p>
                                    </>}
                            ...
                            : <div className={`flex flex-col leading-[1]`}>
                                {(res.primary_text && res.secondary_text && !isTop) && <p className={`flex items-center line-clamp-1 ${primary_pb}`}><span className={`primary_text pr-[8px] line-clamp-1 ${nunito.className} flex-[0_0_calc(50%_-_5px)]`}>{res.primary_text}</span> {res.secondary_text && <span className='h-[10px] w-[1px] bg-[#6f6f6f] '></span>} <span className={`secondary_text ${nunito.className} line-clamp-1 pl-[8px] flex-[0_0_calc(50%_-_5px)]`}>{res.secondary_text}</span></p>}
                                {res.title && <h6 className={`title  pt-[5px] ${titleClamp ? titleClamp : 'line-clamp-1'} ${nunito.className}`}>{res.title ? res.title : ''}</h6>}
