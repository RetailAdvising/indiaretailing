...
export default function ImageGroupEvents({ data, height, width,isHome}) {
    // console.log(data,"data")
    return (
        <>
            {data && data.map((res, index) => {
                return (
                    <Link key={index} href={isHome + res.route }>
                        <div className={`imageGroup h-full`} >
                            ...
                            <div className={`pt-[10px]`}>
                                ...
                                <div className={`flex gap-[10px] items-center pt-[5px]`}><h6 className={`font-[700] text-[16px] flex-[0_0_calc(70%_-_10px)] capitalize line-clamp-2 nunito`}>{res.title} </h6>
                                    <p className={`flex gap-[5px] flex-[0_0_calc(30%_-_10px)]`}><Image src={'/calendar.svg'} objectFit='contain' className='h-[20px] w-[20px]' height={15} width={20} alt={res.title} /> <span className={`light_text`}>{res.event_date}</span></p>
