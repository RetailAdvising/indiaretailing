                ...
                return (
                    <Link key={index} href={isHome ? isHome + res.route : null}>
                        <div className={`imageGroup h-full`} >
                            ...
                            <div className={`contents pt-[10px]`}>
                                {/* <p className={`primary_text `}>{res.primary_text} </p> */}
                                <div className={`flex gap-[10px] items-center pt-[5px]`}><p className={`font-semibold text-[20px] flex-[0_0_calc(75%_-_10px)]`}>{res.title} </p>
                                    <p className={`flex gap-[5px] flex-[0_0_calc(25%_-_10px)]`}><Image src={'/calendar.svg'} objectFit='contain' className='h-[25px] w-[25px]' height={15} width={20} alt={res.title} /> <span className={`light_text`}>{res.start_date}</span></p>
