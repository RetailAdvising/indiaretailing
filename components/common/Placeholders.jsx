                        ...
                        <div key={index} className={`my-[10px]`}>
                            ...
                            {(resp.placeholder_type == 'list' && resp.data && resp.data.length != 0) &&
                                <div className='border rounded-[5px] p-[10px_15px_15px]'>
                                    <Title data={resp} />
                                    {resp.title == 'Product' ? <List  line_clamp={'3'} flex={'mb-[10px]'}  check={true} boxShadow={true}  imgWidth={'flex-[0_0_calc(30%_-_10px)] md:flex-[0_0_calc(35%_-_10px)]'} height={'h-[85px] object-contain'} width={'w-full'} data={resp.data} /> 
                                    : <Lists flex={'mb-[10px]'} titleClamp={'line-clamp-2'} borderRadius={'rounded-[5px]'} imgFlex={'flex-[0_0_calc(35%_-_10px)]'} tittleOnly={true} check={true} isBB={true} data={resp.data} imgHeight={'h-[85px]'} imgWidth={'w-full'} />}
                                </div>
