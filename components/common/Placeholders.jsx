                ...
                placeholder.map((resp, index) => {
                    return (
                        <div key={index} className={`my-[10px]`}>
                            {(tagbasedAd && Object.keys(tagbasedAd).length != 0 && index==0) ? <Advertisement position={"small"} data={tagbasedAd} /> : (resp.placeholder_type == 'Banner Ad' && resp.data && resp.data.length != 0 && resp.data[0].web_image ) ? <Advertisement divClass={`h-[250px] w-[300px] m-auto`} position={"small"} data={resp.data[0]} height={'h-[250px]'} width={'w-[300px]'} /> : ads_data && ads_data.right_first ? <Advertisement data={ads_data.right_first} position={"small"} divClass={`h-[250px] w-[300px] m-auto`} /> : <></>}
