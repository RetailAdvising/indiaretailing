                ...
                placeholder.map((resp, index) => {
                    return (
                        <div key={index} className={`my-[10px]`}>
                            {(tagbasedAd && Object.keys(tagbasedAd).length != 0 && index == 0) ? <Advertisement ad_payload={ad_payload} position={"small"} insStyle={"display:inline-block;width:300px;height:250px;"} data={tagbasedAd} /> : (resp.placeholder_type == 'Banner Ad' && resp.data && resp.data.length != 0 && resp.data[0].web_image && index == 0) ? <Advertisement insStyle={"display:inline-block;width:300px;height:250px;"} divClass={`h-[250px] w-[300px] m-auto`} position={"small"} data={resp.data[0]} height={'h-[250px]'} width={'w-[300px]'} /> :
                                (ads_data && ads_data.right_first && index == 0) ? <Advertisement ad_payload={ad_payload} data={ads_data.right_first} position={"small"} insStyle={"display:inline-block;width:300px;height:250px;"} divClass={`h-[250px] w-[300px] m-auto`} /> : <></>}
                            ...
                            {(resp.placeholder_type == 'google_ad') && <Advertisement ad_payload={ad_payload} position={"small"} data={((resp.data && resp.data.length != 0) && (resp.data[0] && resp.data[0].script)) ? resp.data[0].script : null} insStyle={"display:inline-block;width:300px;height:250px;"} divClass={`h-[250px] w-[300px] m-auto`} />}
