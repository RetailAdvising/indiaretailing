            ...
            {data && data.map((res, index) => {
                return (
                    ...
                    <div key={index} onClick={() => router.push(`${isHome ? isHome + res.route : '/' + router.asPath.split('/')[1] + '/' + res.route}`)} className={`${flex} flex cursor-pointer gap-3 ${index != data.length - 1 ? 'pb-[10px]' : ''} relative ${exclusives.card_item} ${(isReverse) ? 'flex-row-reverse items-center  mb-[10px] justify-between' : ''} ${(isReverse && index != data.length - 1) ? 'border_bottom' : ''} ${(isBB && index != data.length - 1) && 'border_bottom mb-[10px]'}`}>
                        {(res.primary_text && res.secondary_text && isTop) && <p className={`flex line-clamp-1  ${exclusives.title_top}  items-center absolute`}><span className='primary_text pr-[8px] '>{res.primary_text}</span> <span className='h-[10px] w-[1px] bg-[#6f6f6f]'></span> <span className='pl-[8px] secondary_text'>{res.secondary_text}</span></p>}
                        {/* ${check ? '' : 'basis-1/4'} */}
                        ...
                        <div className={`${(!fullWidth && !isReverse) && ''} ${contentWidth} flex flex-col leading-[1] ${isTop && 'pt-[25px]'}`}>
                            {(res.primary_text && res.secondary_text && !isTop) && <p className={`flex items-center line-clamp-1 ${primary_pb}`}><span className='primary_text pr-[8px] '>{res.primary_text}</span> <span className='h-[10px] w-[1px] bg-[#6f6f6f]'></span> <span className='secondary_text pl-[8px]'>{res.secondary_text}</span></p>}
