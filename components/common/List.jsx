            ...
            {data && data.map((res, index) => {
                ...
                return (
                    ...
                    <div key={res.title ? res.title : index} onClick={($event) => navigate($event, res)} className={`${flex} flex cursor-pointer gap-[15px] ${(index != data.length - 1 && !isMp) ? 'pb-[10px]' : (isMp && index != data.length - 1) ?
                        ...
                        {((res.primary_text || res.secondary_text) && isTop) && <p className={`flex line-clamp-1  ${exclusives.title_top}  items-center absolute`}><span className={`primary_text pr-[8px] line-clamp-1 ${nunito.className}`}>{res.primary_text}</span> {res.secondary_text && <span className='h-[10px] w-[1px]  bg-[#6f6f6f]'></span>} <span className={`pl-[8px] line-clamp-1 secondary_text ${nunito.className}`}>{res.secondary_text}</span></p>}
                        {/* ${check ? '' : 'basis-1/4'} */}
                        ...
                        <div className={`${(!fullWidth && !isReverse) && ''} ${contentWidth} flex flex-col leading-[1] ${isTop && 'pt-[25px]'}`}>
                            {((res.primary_text || res.secondary_text) && !isTop) && <p className={`flex items-center line-clamp-1 ${primary_pb}`}><span className={`primary_text pr-[8px] line-clamp-1 ${nunito.className}`}>{res.primary_text}</span> {res.secondary_text && <span className='h-[10px] w-[1px] bg-[#6f6f6f] '></span>} <span className={`secondary_text line-clamp-1 pl-[8px]  ${nunito.className}`}>{res.secondary_text}</span></p>}
