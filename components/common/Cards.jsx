        ...
        return (
          <div key={index} onClick={()=> router.push(`/${router.asPath.split('/')[1]}/${res.route}`)} className={`${styles.cards} ${flex} cursor-pointer ${isBorder&& 'border rounded-[10px]'} ${cardClass}`} >
            ...
            <div className={`${styles.content} ${isBorder && 'p-[10px] '} ${contentHeight}  flex mobile-flex  justify-between flex-col`}>
              {((res.primary_text && res.secondary_text) && !noPrimaryText) && <p className='flex mobile-flex gap-2 '><span className='primary_text fnt_13'>{res.primary_text}</span> <span className='h-[15px] w-[2px] bg-[#6f6f6f]'></span> <span className='secondary_text'>{res.secondary_text}</span></p>}
