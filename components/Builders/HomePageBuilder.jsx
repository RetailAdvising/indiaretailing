                                    ...
                                    {item.components.map((resp, index) => {
                                        return (
                                            <div className={`${resp.class}`} key={index}>
                                                ...
                                                {(resp.component_title == "EventList" && resp.component_type == "event1" && resp.data) && <div className={`flex flex-wrap p-[30px] gap-[10px_20px]`}><EventList data={resp.data.data} height={"h-[110px]"} width={"w-full"} flex={'flex-[0_0_calc(50%_-_10px)] md:flex-[0_0_calc(100%_-_10px)]'} isHome={true} imageBackground={"#f7f7f7"} /></div>}
