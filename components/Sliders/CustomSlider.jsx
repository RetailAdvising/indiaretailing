                ...
                {data && data.map((res, index) => {
                    return (
                        <Link key={index} className={`${cardClass} border rounded-[10px] overfow-hidden`} href={route ? route + res.route : '/' + router.asPath.split('/')[1] + '/' + res.route}>
                            <div className={``} >
                                <Image loading="lazy" blurDataURL={'/empty_state.svg'} placeholder='blur' className={`${imgClass} rounded-[10px_10px_0_0]`} src={res.thumbnail_image ? check_Image(res.thumbnail_image) : res.image} height={200} width={300} alt={index + 'image'} />
                            </div>
