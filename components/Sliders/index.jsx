...
export default function Sliders({ data,perView,imgClass,event }) {
  console.log(data)
        ...
        {data.map((res, index) => {
          return (
            <div onClick={()=> router.push(`/${router.asPath.split('/')[1]}/category/${res.route}`)} className={`${event ? '' : 'flex items-center justify-center gap-[15px] h-full'}`} key={index}>
              ...
              {/* h-[310px] w-[210px] */}
                <Image alt={''} src={check_Image(res.image)} quality={100} height={300} width={1500} className={`${imgClass ? imgClass : 'h-[300px] w-full'} object-contain`} />
              </div>
