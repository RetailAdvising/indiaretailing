      ...
      <RootLayout isLanding={true} is_detail={true} adIdH={router.query.deatil + 'aH'} adIdF={router.query.deatil + 'aF'} homeAd={advertisement ? advertisement : null} head={''}>
        ...
        {(values && values.length != 0) ? <>
          ...
          {values.map((res, index) => {
            return (
              <div id={'div' + index} key={index + 'aertind'} className={`box ${'div' + index}`} onClick={(e) => {
                if (e.target && e.target.id && e.target.id == ('cmt' + res.route)) {
                  // console.log(e, "e")
                  // let ele = document.getElementById('cmt' + res.route)
                  // // ele.scrollIntoView({ block: 'center', behavior: 'smooth', inline: 'nearest' })
                  // const rect = ele.getBoundingClientRect();
                  // const isCentered =
                  //   rect.top >= window.innerHeight / 4 &&
                  //   rect.bottom <= window.innerHeight * (3 / 4);

                  // if (!isCentered) {
                  //   // Only scroll if it's not centered
                  //   ele.scrollIntoView({
                  //     block: 'center',
                  //     behavior: 'smooth',
                  //     inline: 'nearest',
