...
export default function RootLayout({ children, checkout, isLanding, head, homeAd, data, header_data, is_detail }) {
  ...
  return (
    <>
      ...
      {(!checkout || is_detail) && <div className="md:hidden lg:grid lg:justify-center"><Advertisement data={(homeAd && homeAd.header) && homeAd.header} divClass={'h-[90px] w-[728px] m-auto'} insStyle={"display:inline-block;width:728px;height:90px;"} position={"high"}  /></div>}
      {/* <PdfViewer/> */}
      <>
        ...
        {(!checkout && !is_detail) && <div className="md:hidden mb-[10px] lg:grid lg:justify-center"><Advertisement data={(homeAd && homeAd.footer) && homeAd.footer} position={"high"} divClass={'h-[90px] w-[728px] m-auto'} insStyle={"display:inline-block;width:728px;height:90px;"} style={styles} height={'h-full'} width={'500px'} /></div>}
